...
def create_evaluation_splits(n_sample,
    ...
    if year_idx:
        ...
        train, test = train_test_split(np.arange(n_sample),
...
def aggregate(output):
    ...
    for dirpath, dirnames, filenames in os.walk(output):
        if not dirnames:
            files += [
                ...
                for file in filenames if file.endswith("pred.eval.mean.csv")
            ]
    ...
    for rf, r in rfiles:
        ...
        for rd, rr in rdomains:
            ...
            dff.to_csv(f"{output}{rd}/{rf.replace('.csv', '.agg.csv')}",
                       index=False)


def run(data_list, domain_list, fair, filter, future, model_list, output,
        exp_id, settings):
    ...
    if 'tfnn_a1' in model_list:
        models['tfnn_a1'] = tNtf(Fnn(),
                                 settings['model']['nfolds'],
                                 step_ahead=settings['model']['step_ahead'])
    if 'tbnn_a1' in model_list:
        models['tbnn_a1'] = tNtf(Bnn(),
                                 settings['model']['nfolds'],
                                 step_ahead=settings['model']['step_ahead'])
    ...
    if 'rrn' in model_list:
        models['rrn'] = Rrn(settings['model']['baseline']['rrn']['with_zero'],
                            settings['model']['step_ahead'])
    ...
    for (d_name, d_cls) in datasets.items():
        ...
        for i in range(1, len(indexes['i2y'])):
            if indexes['i2y'][i][0] - indexes['i2y'][
                    i - 1][0] > settings['model']['nfolds']:
        ...
        for (m_name, m_obj) in models.items():
            ...
            baseline_name = m_name.lstrip('t').replace('_emb', '').replace(
                '_dt2v', '').replace('_a1', '')
...
def addargs(parser):
    ...
    dataset.add_argument(
        ...
        required=True,
        help=
        'a list of dataset paths; required; (eg. -data ./../data/raw/toy.json)'
    )
    dataset.add_argument(
        '-domain',
        '--domain-list',
    ...
    baseline.add_argument(
        ...
        required=True,
        help=
        'a list of neural models (eg. -model random fnn bnn fnn_emb bnn_emb nmt)'
    )
    ...
    fair.add_argument(
        ...
        required=False,
        help=
        'desired ratio of non-popular experts after reranking; if None, based on distribution in dataset; default: None; Eg. 0.5'
    )
    fair.add_argument(
        '-fairness',
        '--fairness',
        ...
        required=False,
        help=
        'reranking algorithm from {det_greedy, det_cons, det_relaxed}; required; Eg. det_cons'
    )
    fair.add_argument(
        '-k_max',
        '--k_max',
