...
def run(data_list, domain_list, fair, filter, future, model_list, output, exp_id, settings):
    ...
    for (d_name, d_cls) in datasets.items():
        ...
        for (m_name, m_obj) in models.items():
            ...
            output_path = f"{output}\{d_name}\{os.path.split(datapath)[-1]}{filter_str}/{m_name}/t{vecs_['skill'].shape[0]}.s{vecs_['skill'].shape[1]}.m{vecs_['member'].shape[1]}.{'.'.join([k + str(v).replace(' ', '') for k, v in settings['model']['baseline'][baseline_name].items() if v])}"
