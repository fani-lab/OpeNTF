...
function fetchData() {
  ...
  return fetch(`http://localhost:5000/${selectedDataset}/model_data.json`)
    .then(response => {
      if (response.ok) {
        return response.json();
      } else {
...
function updateChartWithData(modelData, metric) {
  ...
  if(r != 12){
    ...
    const maxValue = Math.max(...data_2, ...data_5, ...data_10);
    var steps = Math.ceil(maxValue / 0.1);
  } else{
    const data = checkedData.map(row => parseFloat(row[r+1]).toFixed(3));
    barChartData.datasets = [
      {
        label: metric,
        fillColor: colors[1],
  ...
  .then(data => {
    ...
    modelData.forEach((row, index) => {
      ...
      checkbox.onchange = function() {
          updateChartWithData(modelData, selectedMetricValue);
      };
...
document.getElementById("logo").addEventListener("click", function() {
  window.location.href = "https://hosseinfani.github.io/lab/";
});
