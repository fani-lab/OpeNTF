...
class tNtf(Ntf):
    ...
    def learn(self, teamsvecs, indexes, splits, cfg, prev_model, output):
        ...
        for i, v in enumerate(year_idx[:-self.step_ahead]):#the last years are for test.
            ...
            with open(f'{output_}/splits.json', 'wb') as f: pickle.dump(splits, f)

            self.model.learn(teamsvecs, indexes, splits, cfg, prev_model, output_) #not recursive, but fine-tune over years
