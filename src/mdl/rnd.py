    ...
    def test(self, teamsvecs, splits, testcfg):
        ...
        for pred_set in (['test', 'train', 'valid'] if testcfg.on_train else ['test']):
            for foldidx in splits['folds'].keys():
                ...
                Ntf.torch.save({'y_pred': Ntf.to_topk_sparse(y_pred, testcfg.topK) if (testcfg.topK and testcfg.topK < y_pred.shape[1]) else y_pred, 'uncertainty': None}, f'{self.output}/f{foldidx}.{pred_set}.pred', pickle_protocol=4)
