...
class GCNLayer(torch.nn.Module):
    def __init__(self, hidden_channels, add_self_loops = False):
        super().__init__()

        self.conv1 = GCNConv(hidden_channels, hidden_channels, add_self_loops = add_self_loops)
...
class Model(torch.nn.Module):
    ...
    def __init__(self, hidden_channels, data):
        ...
        self.gcnlayer = GCNLayer(hidden_channels)

        if (type(data) == HeteroData):
            # Convert gs model into a heterogeneous variant:
            self.gcnlayer = to_hetero(self.gcnlayer, metadata=data.metadata())
    ...
    def forward(self, data, is_directed) -> Tensor:
        ...
        if(type(data) == HeteroData):
            ...
            for i, node_type in enumerate(data.node_types):
                ...
                x_dict = self.gcnlayer(x_dict, data.edge_index_dict)
