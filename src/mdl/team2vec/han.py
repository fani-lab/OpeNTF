...
class Model(torch.nn.Module):
    ...
    def __init__(self, hidden_channels, data):
        ...
        self.han = HAN(hidden_channels, data.metadata())

        # if (type(data) == HeteroData):
        #     # Convert gs model into a heterogeneous variant:
        #     self.han = to_hetero(self.han, metadata=data.metadata())
    ...
    def forward(self, data, seed_edge_type, is_directed, emb = False) -> Tensor:
        if(type(data) == HeteroData):
            ...
            self.x_dict = self.han(self.x_dict, data.edge_index_dict)
