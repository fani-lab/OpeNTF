...
class Model(torch.nn.Module):
    ...
    def __init__(self, hidden_channels, data):
        ...
        self.gatv2 = GATv2(hidden_channels)

        if (type(data) == HeteroData):
            # Convert gs model into a heterogeneous variant:
            self.gatv2 = to_hetero(self.gatv2, metadata=data.metadata())
    ...
    def forward(self, data, seed_edge_type, is_directed, emb = False) -> Tensor:
        if(type(data) == HeteroData):
            ...
            self.x_dict = self.gatv2(self.x_dict, data.edge_index_dict)
            ...
            x = self.gatv2(x, data.edge_index)
            self.x = x
