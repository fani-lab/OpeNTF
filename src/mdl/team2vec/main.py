...
def addargs(parser):
    """
    Add arguments to the provided argparse parser for Team Embedding.

    Parameters:
        parser (argparse.ArgumentParser): The argparse parser object to which arguments will be added.
    ...
    args = [
        ...
        },
        {
            ...
            "type": str,
            "required": True,
            "help": "The embedding model; e.g., w2v, n2v, ...",
        },
        {
            "flag": "-output",
            "type": str,
    ...
    parser.add_argument_group("Team Embedding")
    for arg in args:
        flag = arg.pop("flag")
...
def run(teamsvecs_file, indexes_file, model, output):
    if not os.path.isdir(output):
        os.makedirs(output)
    with open(teamsvecs_file, "rb") as teamsvecs_f, open(
        indexes_file, "rb"
    ) as indexes_f:
        ...
        if model == "w2v":
            ...
            wnn.run(
                f"{args.teamsvecs}teamsvecs.pkl",
                f"{args.teamsvecs}indexes.pkl",
                settings,
                output_,
            )
            # or
        ...
        output_ = (
            output
            + f'{params.settings["graph"]["edge_types"][1]}.{"dir" if params.settings["graph"]["dir"] else "undir"}.{str(params.settings["graph"]["dup_edge"]).lower()}/'
        )
        ...
        if model == "gnn.n2v":
            ...
            t2v.model = Node2Vec(
                ...
                num_negative_samples=params.settings["model"][model][
                    "num_negative_samples"
                ],
            ).to(t2v.device)

            t2v.loader = t2v.model.loader(
                batch_size=params.settings["model"]["batch_size"],
                shuffle=params.settings["model"]["loader_shuffle"],
                num_workers=params.settings["model"]["num_workers"],
            )
            t2v.optimizer = torch.optim.Adam(
                list(t2v.model.parameters()), lr=params.settings["model"]["lr"]
            )
            t2v.model_name = "n2v"
        ...
        # gcn (for homogeneous only)
            ...
            t2v.optimizer = torch.optim.Adam(
                t2v.model.parameters(), lr=params.settings["model"]["lr"]
            )
            t2v.model_name = "gcn"

        t2v.train(
            params.settings["model"]["max_epochs"],
            params.settings["model"]["save_per_epoch"],
        )
        t2v.plot_points()
        print(t2v)
...
def test_toys(args):
    ...
    for args.teamsvecs in [
        "./../../../data/preprocessed/dblp/toy.dblp.v12.json/",
        ...
        for edge_type in [
            ("member", "m")
        ]:  # n2v is only for homo, ([('skill', '-', 'team'), ('member', '-', 'team')], 'stm'), ([('skill', '-', 'member')], 'sm')]:
