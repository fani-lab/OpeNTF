import subprocess, os, re, shlex, numpy as np, logging, sys, scipy, shutil
    ...
    def test(self, teamsvecs, splits, testcfg):
        ...
        for foldidx in splits['folds'].keys():
            ...
            for modelfile in modelfiles:
                ...
                cli_cmd += f'-output {self.output}/f{foldidx}.test.e{step}.pred_ '
            ...
            predfiles = [_ for _ in os.listdir(self.output) if re.match(f'f{foldidx}.test.e\d+.pred_$', _)]
            ...
            for i, predfile in enumerate(predfiles):
                ...
                # TODO: sparse?
                Y_ = Ntf.torch.zeros((Y.shape[0], Y.shape[1]))
                for _ in range(Y.shape[0]):
                    ...
                    for pred in predlist: Y_[_, int(pred)] = 1 / len(set(predlist)) # seq2seq may repeat a token, here an expert

                Y_ = opentf.topk_sparse(Ntf.torch, Y_, testcfg.topK) if (testcfg.topK and testcfg.topK < Y_.shape[1]) else Y_
                torchfile = f'{self.output}/{predfile.replace(".pred_", ".pred")}'
                Ntf.torch.save({'y_pred': Y_, 'uncertainty': None}, torchfile, pickle_protocol=4)
                if i == 0: shutil.copy2(torchfile, f'{self.output}/f{foldidx}.test.pred') # the first in the reverse-sorted list is the last epoch, so we need a new file to be consistent with other models

    def evaluate(self, teamsvecs, splits, evalcfg):
        evalcfg.on_train=None # we cannot have on_train or on_valid for nmt
        super().evaluate(teamsvecs, splits, evalcfg)
