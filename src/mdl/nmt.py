...
class Nmt(Ntf):
    ...
    def build_vocab(self, input_data, output_data, splits, settings, model_path):
        endl = "\n"
        input_data = np.array(
            ["{}{}".format(_, endl) for _ in [" ".join(_) for _ in input_data]]
        )
        output_data = np.array(
            ["{}{}".format(_, endl) for _ in [" ".join(_) for _ in output_data]]
        )
        ...
        for foldidx in splits["folds"].keys():
            ...
            settings["save_checkpoint_steps"] = (
                500  # overrides per_epoch evaluation and it becomes per_checkpoints
            )
