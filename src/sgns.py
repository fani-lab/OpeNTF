...
def learn(splits, indexes, vecs, params, output, unigram):
    ...
    num_nodes = params['d']; learning_rate = params['lr']; batch_size = params['b']; num_epochs = params['e'];ns = params['ns']
    ...
    for foldidx in splits['folds'].keys():
        ...
        # Initialize network
        model = SGNS(input_size=input_size, output_size=output_size, param=params).to(device)
            ...
            for phase in ['train', 'valid']:
                for batch_idx, (X, y) in enumerate(data_loaders[phase]):
                    ...
                    if phase == 'train':
                        ...
                        y_ = model(X)
                        if params['s'] == "uniform":
                            loss = sgns_with_logits(y_, y, ns)
                        elif params['s'] == "unigram":
                            loss = sgns_with_logits_with_unigram(y_, y, unigram, ns)
                        else:
                            loss = sgns_with_logits_with_mini_batch_unigram(y_, y, ns)
                            # backward
                        loss.backward()
...
def main(splits, vecs, indexes, output, settings, cmd):
    ...
    output = f"{output}/t{vecs['skill'].shape[0]}.s{vecs['skill'].shape[1]}.e{vecs['member'].shape[1]}.{'.'.join([k + str(v) for k,v in settings.items()])}"

    if 'train' in cmd:
        learn(splits, indexes, vecs, settings, output, unigram)
    ...
    if 'eval' in cmd:
        eval(output, splits, indexes, vecs, settings)

    if 'test' in cmd:
        test(output, splits, indexes, vecs, settings)