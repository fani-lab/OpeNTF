import csv, requests, traceback, time, os
...
class Repo(Team):
    ...
    def crawl_repo(writer, reader, GET_header):
        ...
        try:
            ...
            if len(collabs) > 1: writer.writerow({'repo': repo, 'collabs': collabs, 'langs': langs, 'rels': rels, 'pushed_at': pushed_at,
...
Repo.crawl_github(input='./../../data/raw/gith/toy.repos.csv', output='./../../data/raw/gith/data.csv', access_token='', nseconds=8)