...
class CustomUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        ...
        try:
            ...
            DummyClass = type(name, (), {
                '__init__': lambda self, *args, **kwargs: setattr(self, 'args', (args, kwargs)),
...
class TqdmProgressFileReader:
    ...
    def __init__(self, file_path, pbar=None):
        ...
        if self.pbar is None:
            self.pbar = tqdm(total=self.file_size, unit='B', unit_scale=True, 
        ...
        if fmt == 'pickle':
            ...
            with open(file_path, 'rb') as f:
                try:
                    ...
                    print(f"Warning: {str(e)} - Attempting to load with custom module handling")
                    f.seek(0)  # Reset file pointer
                    try:
                        data = CustomUnpickler(f).load()
                    except Exception as inner_e:
                        print(f"Error loading pickle with custom handler: {str(inner_e)}")
                        raise
            if pbar:
            ...
            for enc in encodings_to_try:
                try:
                    ...
                    if batch_size:
                        if pbar:
                            pbar.update(10)
                        data = pd.read_csv(file_path, chunksize=batch_size, encoding=enc)
                        if pbar:
                            pbar.set_description(f"Successfully loaded CSV with {enc} encoding")
            ...
            raise last_exception or ValueError("Failed to read CSV file with any encoding")
            ...
            for enc in encodings_to_try:
                try:
                    ...
                    if batch_size:
                        if pbar:
                            pbar.update(10)
                        data = pd.read_csv(file_path, sep='\t', chunksize=batch_size, encoding=enc, low_memory=False)
                        if pbar:
                            pbar.set_description(f"Successfully loaded TSV with {enc} encoding")
                            pbar.update(80)  # Complete progress for setup
                        return data
                    else:
                        # Use progress bar if available
                        if pbar:
                            pbar.update(10)
                        data = pd.read_csv(file_path, sep='\t', encoding=enc, low_memory=False)
                        if pbar:
                            pbar.set_description(f"Successfully loaded TSV with {enc} encoding")
                            pbar.update(80)  # Complete progress
                        return data
            ...
            # If we get here, all encodings failed
            raise last_exception or ValueError("Failed to read TSV file with any encoding")
                    ...
                    with open(file_path, 'r', encoding=enc) as f:
                        try:
                            ...
                            try:
                                ...
                                is_jsonl = True
                                if pbar:
                                    pbar.set_description(f"Detected JSON Lines format (encoding: {enc})")
                                    pbar.update(5)
                            except json.JSONDecodeError:
                                is_jsonl = False
                                if pbar:
                                    pbar.set_description(f"Detected standard JSON format (encoding: {enc})")
                                    pbar.update(5)
                            ...
                            if is_jsonl:
                                ...
                                if pbar:
                                    pbar.set_description(f"Counting lines in JSONL file (encoding: {enc})")
                                    pbar.update(5)
                                ...
                                if file_size > 1024*1024*100:  # 100MB
                                    ...
                                    with open(file_path, 'r', encoding=enc) as sample_f:
                                        sample_data = sample_f.read(1024*1024)  # Read 1MB
                                        line_count_sample = sample_data.count('\n')
                                        lines_per_mb = line_count_sample / (len(sample_data) / (1024*1024))
                                        estimated_lines = int(file_size / (1024*1024) * lines_per_mb)
                                        if pbar:
                                            pbar.set_description(f"Estimated {estimated_lines} lines in large JSONL file")
                                            pbar.update(10)
                                else:
                                    # Count exact lines for smaller files
                                    estimated_lines = sum(1 for _ in open(file_path, 'r', encoding=enc))
                                    if pbar:
                                        pbar.set_description(f"Found {estimated_lines} lines in JSONL file")
                                        pbar.update(10)
                                ...
                                if pbar:
                                    pbar.set_description(f"Reading standard JSON (encoding: {enc})")
                                    pbar.update(10)
                                ...
                                if pbar:
                                    pbar.set_description(f"JSON data loaded with {enc} encoding")
            ...
            # If we get here, all encodings failed
            raise last_exception or ValueError(f"Failed to read JSON file with any encoding")
        else:
...
def count_matches(data, count_filters):
    ...
    unique_by_values = defaultdict(lambda: defaultdict(set))  # For uniqueBy functionality
    ...
    special_filters = [(field, value, should_split, split_by, unique_by, count_all) 
                       for field, value, should_split, split_by, unique_by, count_all in count_filters 
                       if value.lower() in ['unique', 'all']]
    specific_filters = [(field, value, should_split, split_by, unique_by, count_all) 
                        for field, value, should_split, split_by, unique_by, count_all in count_filters 
                        if value.lower() not in ['unique', 'all']]
    
    # Check if we're counting titles
    counting_title = any(field.lower() == 'title' for field, _, _, _, _, _ in special_filters)
    ...
    print(f"Processing {len(unique_filters)} unique value filters, {len(all_filters)} all value filters, and {len(specific_filters)} specific value filters")
    ...
    def matches_specific_filters(record):
        ...
        return all(
            field in record and (
                (should_split and ',' in str(record[field]) and 
                 value.lower() in [v.strip().lower() for v in str(record[field]).split(',')]) or
                (not should_split and not split_by and not unique_by and 
                 str(record[field]).lower() == value.lower())
            )
    ...
    def process_unique_by_value(field_name, field_value, unique_by_field, count_all=False):
        if field_value is None or not isinstance(field_value, list):
            return
                    ...
                    if unique_key_lower not in unique_by_tracking[field_name]:
                        ...
                        if field_name in item:
                            ...
                            if count_all:
                                ...
                                unique_by_values[field_name][target_value_lower].add(unique_key_lower)
    ...
    if hasattr(data, 'get_chunk'):
        ...
        with tqdm(total=batch_count, desc="Processing batches", unit="batch", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as batch_pbar:
            batch_num = 0
            for batch in data:
                batch_num += 1
                ...
                for record in batch_records:
                    ...
                    if matches_specific_filters(record):
                        ...
                        for field, value, should_split, split_by, unique_by, count_all in special_filters:
                            if field in record:
                                if unique_by:
                                    process_unique_by_value(field, record[field], unique_by, count_all)
                                elif split_by:
                                    process_nested_value(field, record[field], split_by, count_all)
                                else:
                                    process_value(field, record[field], should_split, count_all)
                        
                        # Process specific filters
                        for field, value, should_split, split_by, unique_by, count_all in specific_filters:
                            if field in record:
                                if should_split and ',' in str(record[field]):
                                    if value.lower() in [v.strip().lower() for v in str(record[field]).split(',')]:
                                        counts[f"{field}={value}"] += 1
                                elif split_by and isinstance(record[field], list):
                                    for item in record[field]:
                                        if isinstance(item, dict) and split_by in item and str(item[split_by]).lower() == value.lower():
                                            counts[f"{field}={value}"] += 1
                                elif unique_by and isinstance(record[field], list):
                                    process_unique_by_value(field, record[field], unique_by, count_all)
                                elif not split_by and not unique_by and str(record[field]).lower() == value.lower():
                                    counts[f"{field}={value}"] += 1
        ...
        with tqdm(total=len(data), desc="Processing records", unit="record", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as record_pbar:
        ...
        with tqdm(total=len(data), desc="Processing items", unit="item", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as item_pbar:
                    ...
                    for field, value, should_split, split_by, unique_by, count_all in special_filters:
                        if field in item:
                            if split_by and isinstance(item[field], list):
                                process_nested_value(field, item[field], split_by, count_all)
                            else:
                                process_value(field, item[field], should_split, count_all)
                    
                    # Process specific filters
                    for field, value, should_split, split_by, unique_by, count_all in specific_filters:
                        if field in item:
                            if should_split and ',' in str(item[field]):
                                if value.lower() in [v.strip().lower() for v in str(item[field]).split(',')]:
                                    counts[f"{field}={value}"] += 1
                            elif split_by and isinstance(item[field], list):
                                for sub_item in item[field]:
                                    if isinstance(sub_item, dict) and split_by in sub_item and str(sub_item[split_by]).lower() == value.lower():
                                        counts[f"{field}={value}"] += 1
                            elif not split_by and not unique_by and str(item[field]).lower() == value.lower():
                                counts[f"{field}={value}"] += 1
                elif isinstance(item, list):
                    # For list items, use a nested progress bar
                    with tqdm(total=len(item), desc=f"Processing {key} list", unit="record", leave=False, bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt}') as subitem_pbar:
                        for record in item:
                            if isinstance(record, dict) and matches_specific_filters(record):
                                # Process unique and all filters
                                for field, value, should_split, split_by, unique_by, count_all in special_filters:
    ...
    if unique_values:
        ...
        print(f"Processing {sum(len(values) for values in unique_values.values())} unique values...")
        with tqdm(desc="Counting unique values", total=len(unique_values), unit="field", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as count_pbar:
    ...
    if all_values_count:
        ...
        print(f"Processing counts for all occurrences ({len(all_values_count)} fields)...")
        with tqdm(desc="Processing all counts", total=len(all_values_count), unit="field", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as all_count_pbar:
            for field, value_counter in all_values_count.items():
                all_count_pbar.set_description(f"Counting {field} all values ({len(value_counter)})")
    ...
    if nested_values:
        print(f"Processing {sum(len(values) for values in nested_values.values())} nested values...")
        with tqdm(desc="Processing nested values", total=len(nested_values), unit="field", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as nested_pbar:
            for field, nested_dict in nested_values.items():
                nested_pbar.set_description(f"Processing {field} nested values ({len(nested_dict)})")
    ...
    if unique_by_values:
        print(f"Processing {sum(len(values) for values in unique_by_values.values())} uniqueBy values...")
        with tqdm(desc="Processing uniqueBy values", total=len(unique_by_values), unit="field", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as unique_by_pbar:
            for field, value_dict in unique_by_values.items():
                unique_by_pbar.set_description(f"Processing {field} uniqueBy values ({len(value_dict)})")
    ...
    if counting_title:
        ...
        print(f"Records with null or \\N title: {records_with_null_title} (counted as '[EMPTY]')")
        print(f"Records with empty title: {records_with_empty_title} (counted as '[EMPTY]')")
        print(f"Total '[EMPTY]' titles: {empty_title_count if empty_title_count > 0 else 'N/A'}")
        ...
        if title_total_count != records_with_title:
            ...
            print(f"total title occurrences ({title_total_count}) indicates there are duplicate titles.")
        
        print("\nNote: All null, \\N, and empty title values are now counted as '[EMPTY]'.")
        print("      The total occurrences count includes all entries (including duplicates).")
...
def filter_by_count_criteria(data, count_filters, max_rows=3):
    ...
    display_filters = [(field, value, should_split, split_by, unique_by, count_all) 
                       for field, value, should_split, split_by, unique_by, count_all in count_filters 
                       if value.lower() not in ['unique', 'all']]
    ...
    field_counts = defaultdict(list)
    with tqdm(total=len(counts), desc="Organizing count results", unit="count") as count_pbar:
        for key, count in counts.items():
            if '=' in key:
                field, value = key.split('=', 1)
                field_counts[field].append((value, count))
    ...
    with tqdm(desc="Filtering data by criteria", leave=False) as filter_pbar:
        if hasattr(data, 'get_chunk'):
            ...
            for record in data:
                if isinstance(record, dict) and all(field in record and (
                        (should_split and ',' in str(record[field]) and 
                         value.lower() in [v.strip().lower() for v in str(record[field]).split(',')]) or
                        (not should_split and not split_by and not unique_by and 
                         str(record[field]).lower() == value.lower())
            ...
            for key, item in data.items():
                if isinstance(item, dict) and all(field in item and (
                        (should_split and ',' in str(item[field]) and 
                         value.lower() in [v.strip().lower() for v in str(item[field]).split(',')]) or
                        (not should_split and not split_by and not unique_by and 
                         str(item[field]).lower() == value.lower())
                    ) for field, value, should_split, split_by, unique_by, count_all in display_filters):
                    rows.append(item)
                    if len(rows) >= max_rows:
                        return rows, max_rows
...
def analyze_data_shape(data):
    """Analyze and display the shape and structure of data."""
    if isinstance(data, pd.DataFrame):
        ...
        if len(data) > 0:
            if isinstance(data[0], dict):
                ...
                print(f"Items are of type: {type(data[0])}")
    else:
        print(f"Data is of type: {type(data)}")

def main():
    # Set up argument parser
    parser = argparse.ArgumentParser(description='Analyze data file structure', 
    ...
    parser.add_argument('-f', '--format', help='Force file format (pickle, csv, tsv, json)')
    parser.add_argument('-p', '--path', help='Additional files to analyze', action='append')
    parser.add_argument('-t', '--threads', type=int, default=multiprocessing.cpu_count(), 
                      help='Number of threads to use (default: number of CPUs)')
    parser.add_argument('-b', '--batch-size', type=int, help='Process files in batches of this size (for large files)')
    parser.add_argument('-c', '--count', help='Count items matching field=value expressions (comma-separated)')
    parser.add_argument('-e', '--encoding', help='Specify file encoding (default: tries utf-8, latin1, cp1252, ISO-8859-1 in that order)')
...
'''
    ...
    with tqdm(total=100, desc="Loading data", unit="%", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as pbar:
    ...
    if count_filters:
        ...
        with tqdm(total=100, desc="Counting matches", unit="%", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as count_pbar:
            # Update at start
            count_pbar.update(5)
            ...
            display_data, max_rows = filter_by_count_criteria(data, count_filters, args.rows)
        ...
        field_counts = defaultdict(list)
        with tqdm(total=len(counts), desc="Organizing count results", unit="item", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as organize_pbar:
            for key, count in counts.items():
                if '=' in key:
                    field, value = key.split('=', 1)
                    field_counts[field].append((value, count))
        ...
        with tqdm(total=len(field_counts), desc="Formatting results", unit="field", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as format_pbar:
            for field, values in field_counts.items():
                # Sort values
                ...
                split_by_field = any((f == field and sb) for f, v, s, sb, ub, ca in count_filters)
                ...
                if field.lower() == "year" or field.lower().endswith("year"):
                    ...
                    if split_by_field:
                        ...
                        if has_more:
                            ...
                            print(f"  ... and {unique_count - display_limit} more unique values")
                        else:
    ...
    if args.path:
        for path in args.path:
            ...
            with tqdm(total=100, desc="Loading data", unit="%", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as pbar:
                additional_data = load_file(path, args.format, args.batch_size, pbar, args.encoding)
            ...
            if count_filters:
                ...
                filtered_data, _ = filter_by_count_criteria(additional_data, count_filters, args.rows)
