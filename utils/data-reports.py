    ...
    try:
        ...
        try:
            ...
            if isinstance(SELECTED_GPU_DEVICES, list):
                ...
                if invalid_devices:
                    tprint(
                        f"Warning: GPU devices {invalid_devices} do not exist (max index: {device_count-1})"
                    )
                    # Remove invalid devices
                    SELECTED_GPU_DEVICES = [
                        d for d in SELECTED_GPU_DEVICES if d < device_count
                    ]
                    if not SELECTED_GPU_DEVICES:
                        tprint(
                            f"No valid GPU devices specified. Falling back to first available GPU."
                        )
                ...
                if SELECTED_GPU_DEVICES:
                    ...
                    for device_idx in SELECTED_GPU_DEVICES:
                        ...
                        device_info.append(
                            f"Device {device_idx}: {props['name'].decode('utf-8')} ({props['totalGlobalMem'] / (1024**3):.1f} GB)"
                        )

                    tprint(
                        f"GPU acceleration enabled. Using {len(SELECTED_GPU_DEVICES)} specified GPU device(s):"
                    )
                ...
                for i in range(device_count):
                    ...
                    device_info.append(
                        f"Device {i}: {props['name'].decode('utf-8')} ({props['totalGlobalMem'] / (1024**3):.1f} GB)"
                    )

                tprint(
                    f"GPU acceleration enabled. Found {device_count} CUDA device(s):"
                )
                for info in device_info:
                    tprint(f"  {info}")
        ...
        tprint(
            "To enable GPU acceleration, install CuPy with: pip install cupy-cuda11x"
        )
...
def analyze_teams(teamsvecs, n_jobs=DEFAULT_THREADS):
    ...
    try:
        ...
        tprint(
            f"Using {n_jobs} threads for analysis (previous NumPy setting: {old_threads})"
        )
    except Exception as e:
    ...
    min_team_experts_list = [
        i for i, count in enumerate(expert_team_counts) if count == min_exp_team
    ]
    max_team_experts_list = [
        i for i, count in enumerate(expert_team_counts) if count == max_exp_team
    ]
    ...
    if n_jobs > 1 and num_teams > 10000:  # Only parallelize for larger datasets
        tprint(
            f"Processing {num_teams} teams in {num_batches} batches using {n_jobs} parallel jobs..."
        )
        ...
        batch_results = Parallel(n_jobs=min(n_jobs, num_batches))(
            ...
            for batch_idx in tqdm(
                range(num_batches), desc="Processing team batches", ncols=100
            )
        )
        ...
        tprint(
            f"Merged results from {num_batches} batches, found {dup_teams} duplicate teams"
        )
    else:
    ...
    tprint(
        f"Found {unique_team_configs} unique team configurations out of {num_teams} teams"
    )
    ...
    with tqdm(
        ...
        for batch_idx in range(num_batches):
            ...
            for i in range(batch_size_actual):
                ...
                if (
                    team_config in seen_configs
                    and seen_configs[team_config] != team_idx
                ):
...
def generate_distribution_charts(
    stats, output_dir, dataset_name, n_jobs=DEFAULT_THREADS, use_gpu=False
):
    """
    ...
    if use_gpu:
        ...
        if gpu_available:
            ...
            tprint(
                f"Using GPU acceleration for distribution charts with {n_jobs} CPU threads for coordination"
            )
            ...
            (
                ...
                stats["skill_indices_per_team"], stats["expert_indices_per_team"]
            )

            # Convert to GPU arrays
            skill_indices_gpu = cp.array(skill_indices, dtype=cp.int32)
            team_indices_for_skills_gpu = cp.array(
                team_indices_for_skills, dtype=cp.int32
            )
            member_indices_gpu = cp.array(member_indices, dtype=cp.int32)
            team_indices_for_members_gpu = cp.array(
                team_indices_for_members, dtype=cp.int32
            )
            ...
            tprint(
                f"Data prepared for GPU in {gpu_prep_time:.2f} seconds: {len(skill_indices)} skill-team pairs, {len(member_indices)} member-team pairs"
            )
            ...
            for batch_idx in tqdm(
                range(num_batches), desc="Counting teams per skill", ncols=100
            ):
                start_idx = batch_idx * batch_size
    ...
    if use_gpu and CUPY_AVAILABLE:
        ...
        if "_gpu" in stats:
            ...
            skill_indices = [
                skill_idx
                for team_skills in stats["skill_indices_per_team"]
                for skill_idx in team_skills
            ]
            team_indices = [
                team_idx
                for team_idx, team_skills in enumerate(stats["skill_indices_per_team"])
    ...
    if not x_values or not y_values:
        tprint(
            "Warning: No data available for teams per skill distribution plot. Skipping."
        )
        return
    ...
    cv = (
        np.std(teams_per_skill) / np.mean(teams_per_skill)
        if len(teams_per_skill) > 0 and np.mean(teams_per_skill) > 0
        else 0
    )
    distribution_type = "Long-tailed" if cv > 1 else "Relatively Uniform"
    plt.annotate(
        f"Distribution type: {distribution_type} (CV={cv:.2f})",
        xy=(0.5, 0.95),
    ...
    with tqdm(
        total=max_skills_to_show, desc="Generating skills heatmap data", ncols=100
    ) as pbar:
    ...
    with tqdm(total=1, desc="Sorting experts by activity", ncols=100) as pbar:
        expert_team_count_pairs = [
            (i, count) for i, count in enumerate(teams_per_expert)
        ]
    ...
    # Annotate if it's long-tailed or uniform
    cv = (
        np.std(teams_per_expert) / np.mean(teams_per_expert)
        if len(teams_per_expert) > 0 and np.mean(teams_per_expert) > 0
        else 0
    )
    distribution_type = "Long-tailed" if cv > 1 else "Relatively Uniform"
    plt.annotate(
    ...
    with tqdm(
        total=max_experts_to_show, desc="Generating experts heatmap data", ncols=100
    ) as pbar:
        for i, expert_idx in enumerate(most_active_experts[:max_experts_to_show]):
            for team_idx in team_indices:
                if expert_idx in stats["expert_indices_per_team"][team_idx]:
                    ...
                    y_points.append(
                        i
                    )  # Use the position in the most_active_experts list for better visualization
            pbar.update(1)
    ...
    try:
        ...
        total_chart_time = (
            time.time() - chart_start_time if "chart_start_time" in locals() else 0
        )
...
def generate_markdown_report(
    stats, dataset_name, filter_info, output_dir, removed_items=None
):
    """
    ...
    """
    # No need to create the directory here - it will be created by the reports directory
    ...
    avg_contributors_per_repo = (
        sum(stats["experts_per_team"]) / stats["num_teams"]
        if stats["num_teams"] > 0
        else 0
    )
    avg_skills_per_repo = (
        sum(stats["skills_per_team"]) / stats["num_teams"]
        if stats["num_teams"] > 0
        else 0
    )
    ...
    teams_with_neither = sum(
        1
        for i in range(stats["num_teams"])
        if i < len(stats["skills_per_team"])
        and i < len(stats["experts_per_team"])
        and stats["skills_per_team"][i] == 0
        and stats["experts_per_team"][i] == 0
    )
...
"""
    ...
    if (
        removed_items
        and any("Duplicate teams:" in item for item in removed_items)
        and stats["dup_teams"] == 0
    ):
        removed_count = next(
            (
                int(item.split(":")[1].strip().replace(",", ""))
                for item in removed_items
                if "Duplicate teams:" in item
            ),
            0,
        )
        if removed_count > 0:
...
def parse_gpu_mode(mode_arg):
    ...
    if mode_arg.startswith("gpu="):
        ...
        try:
            ...
            tprint(
                f"Warning: Invalid GPU device specification '{mode_arg}'. Using first available GPU."
            )
            return "gpu", "first"
...
def main():
    ...
    if args.threads == DEFAULT_THREADS:  # Only if user didn't override
        ...
        if mode == "cpu":
            tprint(
                f"Auto-detected {multiprocessing.cpu_count()} CPU cores, using {args.threads} threads ({int(args.threads/multiprocessing.cpu_count()*100)}% of CPU)"
            )
        else:
            if device_indices == "first":
                tprint(
                    f"Using {args.threads} CPU threads for GPU-accelerated mode with first available GPU"
                )
            elif device_indices == "all":
                tprint(
                    f"Using {args.threads} CPU threads for GPU-accelerated mode with all available GPUs"
                )
            elif isinstance(device_indices, list):
                gpu_str = ", ".join(map(str, device_indices))
                tprint(
                    f"Using {args.threads} CPU threads for GPU-accelerated mode with GPU(s): {gpu_str}"
                )
            else:
    ...
    with tqdm(total=1, desc="Writing report", ncols=100) as pbar:
        with open(output_file, "w") as f:
            ...
            f.write(
                f',{dataset_name.upper()},{stats["num_teams"]},{stats["num_skills"]},{stats["num_experts"]}\n'
            )
            f.write(",,,,\n")
            ...
            zero_expert_percent = (
                stats["zero_expert_teams"] / stats["num_teams"]
            ) * 100
            ...
            min_expert_teams = sum(
                1 for x in stats["expert_team_counts"] if x == stats["min_exp_team"]
            )
            max_expert_teams = sum(
                1 for x in stats["expert_team_counts"] if x == stats["max_exp_team"]
            )
            min_skill_teams = sum(
            ...
            f.write(
                f',{stats["min_exp_team"]} ({min_expert_teams} experts~{min_expert_percent:.1f}%),'
            ...
            min_team_experts = sum(
                1 for x in stats["expert_team_counts"] if x == stats["min_exp_team"]
            )
            max_team_experts = sum(
                1 for x in stats["expert_team_counts"] if x == stats["max_exp_team"]
            )
            ...
            f.write(
                f'- Min teams per expert: {stats["min_exp_team"]} ({min_team_experts} experts ~ {min_exp_percent:.1f}%)\n'
            )
            f.write(
                f'- Max teams per expert: {stats["max_exp_team"]} ({max_team_experts} experts ~ {max_exp_percent:.1f}%)\n\n'
            )
            ...
            with tqdm(
                ...
                for i in range(len(stats["skills_per_team"])):
                    ...
                    skill_indices = [
                        f"s{idx+1}" for idx in stats["skill_indices_per_team"][i]
                    ]
                    expert_indices = [
                        f"m{idx+1}" for idx in stats["expert_indices_per_team"][i]
                    ]

                    f.write(
                        f'{stats["dup_indices"][i]},{stats["skills_per_team"][i]},{len(expert_indices)},'
    ...
    md_file_path = generate_markdown_report(
        stats, dataset_name, filter_info, stats_reports_dir, removed_items
    )
