...
for model in "${models[@]}"; do
  for dataset in "${!datasets[@]}"; do
    ...
    # Get the start time
    start_time=$(date +%s)
    # Get current time, add subtract 3.5 hours to get EST time
    start_time_est=$(date -u -d "+3 hours" +"%Y-%m-%d %H:%M:%S")
    ...
    # Get the end time
    end_time=$(date +%s)
    end_time_est=$(date -u -d "+3 hours" +"%Y-%m-%d %H:%M:%S")
     echo -e "\tEnded at: $(end_time_est)"
